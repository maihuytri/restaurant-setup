...
const UserList = () => {
  ...
  const [isShowMessageBoxModalOpen, setIsShowMessageBoxModalOpen] =
    useState(false);
  ...
  const handleDeleteUser = (user: User | null) => {
    ...
    setTitle("Confirm");
    setMessage(
      `Are you sure do you want to delete this ${selectedUser?.username} ?`
    );
  ...
  const handleOnOk = async () => {
    try {
      ...
      const res = await fetch(
        `${process.env.REACT_APP_APIURL}/users/${selectedUser?.id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
